...
/**
 * Hook to manage phone upload functionality including:
 * - QR code generation
 * - Phone upload server toggle
 * - Token management
...
export const usePhoneUpload = (): UsePhoneUploadReturn => {
  ...
  const phoneUploadBase = useMemo(() => {
    ...
    const separator = phoneUploadBaseUrl.includes('?') ? '&' : '?';
    return `${phoneUploadBaseUrl}${separator}target=photo_journal`;
  }, [phoneUploadBaseUrl]);

  const phoneUploadLink = useMemo(() => {
  ...
  // Generate QR code
  useEffect(() => {
    if (!phoneUploadLink || !phoneUploadEnabled) {
      setQrCodeDataUrl(null);
      setQrStatus('idle');
      return;
    }
    let cancelled = false;
    setQrStatus('loading');
    QRCode.toDataURL(phoneUploadLink, {
      width: 220,
      margin: 1,
  ...
  }, [phoneUploadLink, phoneUploadEnabled]);
